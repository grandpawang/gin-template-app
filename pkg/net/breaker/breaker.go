...
type Config struct {
	...
	// The error rate triggering a fuse (K = 1-1 / error rate)
	K float64

	// Statistics bucket window time
	Window xtime.Duration
	// Statistics bucket size
	Bucket int
	// The minimum number of requests to trigger the fuse
	// (if the request is less than this value, the fuse will not be triggered)
	Request int64
}
...
const (
	...
	StateClosed
	// StateHalfopen when circuit breaker open, after sleep some duration, allow
